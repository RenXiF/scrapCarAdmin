<template>
	...
	<div class="flex_columns ">
		<h6 class="flex_jufy_center" style="font-size: 2.25rem;margin: 0.625rem 0;">用户管理</h6>
		<el-header>
			<el-row :gutter="12" class="flex_wrap">
				...
				<el-col :xs="12" :sm="10" :md="8" :lg="7" :xl="5">
					<el-input placeholder="请输入查询的用户" v-model="searchiut" clearable  @change="searchmod">
		...
		<comfrom :showMsg.sync="show" :fromLabel="Fromlabel" tit="添加用户" :fromModel="FromModel" @conModify="conAddlist"
...
<script>
	...
	export default {
		...
		watch: {},
		created() {
			this.getCaretails(this.pageNum); //获取用户
			this.getdment(); //获取部门
			this.getcatdiG(); //获取岗位
			this.loading = false;
		},
		...
		methods: {
			...
			getdment() { //获取所有部门
				...
				this.$api.dment.dmentG(list, (res) => {
					...
					if (res.code == 0 && res.list.length!=0) {
						this.Fromlabel[0].options = res.list.map(iterator => {
							return {
								label: iterator.bumenName,
								value: iterator.id,
							}
						});
						return;
			...
			getcatdiG() { //获取岗位
				...
				this.$api.dicti.catdiG(list, (res) => {
					console.log(res);
					if (res.code == 0 && res.list.length!=0) {
						this.Fromlabel[1].options = res.list.map(iterator => {
							return {
			...
			addstudent() {
				// console.log(this.FromModel);
				this.$api.staUser.userA(this.FromModel, (res) => {
					console.log(res);
					this.disabled = false;
					if (res.code == 0) {
			...
			getCaretails(pageNum) { //获取用户
				this.loading = true;
				this.tableList = [];
				var list = {
					pageNum: pageNum,
					pageSize: this.pageSize,
					employeeName: this.searchiut,
				};
				console.log(list);
			...
			edit(item) { //编辑接口
				// console.log(item)
				this.$api.staUser.userU(item, (res) => {
					console.log(res);
					this.disabled = false;
					if (res.code == 0) {
						this.$message.success(res.message);
