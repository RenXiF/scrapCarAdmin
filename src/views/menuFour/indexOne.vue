<template>
	...
	<div class="flex_columns ">
		<h6 class="flex_jufy_center" style="font-size: 2.25rem;margin: 0.625rem 0;">仓库管理</h6>
		<el-header>
			<el-row :gutter="12" class="flex_wrap">
				...
				<el-col :xs="12" :sm="10" :md="8" :lg="7" :xl="5">
					<el-input placeholder="请输入查询的仓库" v-model="searchiut" clearable  @change="searchmod">
		...
		<comfrom :showMsg.sync="show" :fromLabel="Fromlabel" tit="添加仓库" :fromModel="FromModel" @conModify="conAddlist"
	...
	export default {
		...
		methods: {
			...
			getCaretails(pageNum) { //获取仓库
				...
				console.log(list);
				this.$api.store.catwareG(list, (res) => {
					console.log(res);
					this.loading = false;
					if (res.code == 0) {
			...
			deleteDete() { //确认删除
				...
				this.$api.store.catwareD(li, (res) => {
					console.log(res);
					if (res.code == 0) {
						this.$message.success(res.message);
						this.getCaretails(this.pageNum); //获取仓库
			...
			edit(item) { //编辑接口
				// console.log(item)
				this.$api.store.catwareU(item, (res) => {
					console.log(res);
					this.disabled = false;
					if (res.code == 0) {
						this.$message.success(res.message);
