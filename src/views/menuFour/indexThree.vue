	...
	<div class="flex_columns ">
		...
		<el-header>
			<el-row :gutter="12" class="flex_wrap">
				...
				<!-- <el-col :xs="8" :sm="6" :md="4" :lg="3" :xl="2">
					<el-select v-model="Letter" placeholder="请选择查询类型" @change="selectCK"
				...
				<el-col :xs="12" :sm="10" :md="8" :lg="7" :xl="5">
					<el-input placeholder="请输入需要查询的零件名称" v-model="searchiut" clearable  @change="searchmod">
			...
			<el-table :data="tableList" height="495" stripe border v-loading="loading">
				...
				<el-table-column label="操作" align="right"  fixed="right" width="170">
					<template slot-scope="scope">
						<!-- <el-button size="mini" @click="Dismantling(scope.$index, scope.row)" v-if="scope.row.state ==0">拆解</el-button> -->
						<!-- <el-button size="mini" :disabled="true" type="success" v-else>已申请</el-button> -->
		...
		<comfrom :showMsg.sync="show" :fromLabel="Fromlabel" tit="拆解" :fromModel="FromModel" @conModify="conAddlist"
...
<script>
	...
	export default {
		...
		watch: {},
		created() {
			this.getCaretails(this.pageNum); //获取拆解车辆
			this.getcatwareG();
			this.loading = false;
		},
		mounted() {
		...
		methods: {
			...
			getCaretails(pageNum) { //获取拆解车辆
				this.loading = true;
				this.tableList = [];
				var list = {
					pageNum: pageNum,
					pageSize: this.pageSize,
					partName: this.searchiut,
